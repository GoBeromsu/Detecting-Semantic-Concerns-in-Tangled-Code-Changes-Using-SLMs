...
Meta-runner that orchestrates ~16 analysis scripts from one config. Not a library — everything flows through `run.py`.
...
- `models`: GPT-4.1, Qwen, QwenFT with CSV path templates `msg{msg}/{context}_zs.csv`.
- `rq1–4`: name, description, per-script `{file, models/csv_files, required_columns}`.

## The four RQs (mind the numbering — CLAUDE.md gets this wrong)
...
Shared helpers: `plot_utils.py` (styling, `display_model_name`), `stats_utils.py` (Wilcoxon/Mann-Whitney), `data_aggregation/{aggregate,average}_experiments.py`.
