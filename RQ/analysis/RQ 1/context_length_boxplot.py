...
def create_context_length_boxplot(csv_data_by_context: dict, output_dir: Path, metric: str = 'hamming_loss') -> Path:
    ...
    for context_length in context_lengths:
        ...
        for model_name in model_names:
            if (context_length in csv_data_by_context and
                model_name in csv_data_by_context[context_length]):
    ...
    for i, model_name in enumerate(model_names):
        ...
        for j, context_length in enumerate(context_lengths):
            if (context_length in plot_data and
                model_name in plot_data[context_length] and
                len(plot_data[context_length][model_name]) > 0):
        ...
        if model_data and positions:
            ax.boxplot(model_data, positions=positions, widths=width*0.8,
                      patch_artist=True,
                      showfliers=True,  # Show outliers (points beyond 1.5*IQR)
                      whis=1.5,        # Standard whisker length (1.5 * IQR)
                      boxprops=dict(facecolor=model_colors[i % len(model_colors)],
                                   alpha=PLOT_STYLE['alpha']),
                      medianprops=dict(color=COLORS['success'],
                                     linewidth=PLOT_STYLE['line_width']),
                      whiskerprops=dict(color=COLORS['text'],
                                      linewidth=PLOT_STYLE['line_width']),
                      capprops=dict(color=COLORS['text'],
                                   linewidth=PLOT_STYLE['line_width']),
                      flierprops=dict(marker='o', markerfacecolor=model_colors[i % len(model_colors)],
    ...
    ax.set_title(f'{metric_label} Distribution by Context Length and Model',
    ...
    legend_elements = [
        Patch(facecolor=model_colors[i % len(model_colors)],
              alpha=PLOT_STYLE['alpha'], label=model_name)
        for i, model_name in enumerate(model_names)
    ]
    ...
    plt.savefig(plot_path, dpi=PLOT_STYLE['dpi'], bbox_inches='tight', facecolor='white')
    plt.close()
...
def main():
    ...
    if args.output_dir:
        ...
        output_dir = (
            ANALYSIS_OUTPUT_DIR / f"context_boxplot_{models_summary}_{timestamp}"
        )
    ...
    # Print data summary
    total_samples = sum(len(df) for context_data in csv_data_by_context.values() for df in context_data.values())
    total_contexts = len(csv_data_by_context)
    print(f"Loaded {total_samples} total samples across {total_contexts} context lengths from {len(script_config['models'])} models")

    # Generate Hamming Loss box plot only
    print("\nGenerating Context Length Hamming Loss box plot...")

    # Generate Hamming Loss box plot
    hamming_plot_path = create_context_length_boxplot(csv_data_by_context, output_dir, 'hamming_loss')
