...
def create_boxplot(df: pd.DataFrame, output_dir: Path) -> Path:
    ...
    # Add regression line (use positions 1-5 for box plot x-axis)
    regression_results = perform_linear_regression(df)
    x_range = np.linspace(min(context_lengths), max(context_lengths), 100)
    y_pred_line = (
...
def main():
    ...
    if args.output_dir:
        ...
        output_dir = (
            ANALYSIS_OUTPUT_DIR / f"it_{files_summary}{outlier_suffix}_{timestamp}"
        )
